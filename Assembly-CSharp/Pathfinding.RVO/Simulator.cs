    ...
    public class Simulator
    {
        ...
        private void PreCalculation()
        {
            ...
            while (true)
            {
                if (num < agents.Count)
                {
                    agents[num].PreCalculation();
                    if (agents[num].MovementMode != agents[0].MovementMode)
                    {
                        break;
                    }
                    num++;
            ...
            Debug.LogError("RVO movement modes are inconsistent. Some are set to " + agents[num].MovementMode + " and some to " + agents[0].MovementMode + ". Local avoidance will not work properly while this is the case.");
        }
