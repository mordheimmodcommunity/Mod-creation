...
public class CharacterFollowCam : ICheapState
{
	...
	private Vector3 regularOffset = new Vector3(0f, 1.5f, 0f);

	private Vector3 skavenOffset = new Vector3(0f, 1.2f, 0f);

	private Vector3 impressiveOffeset = new Vector3(0f, 1.8f, 0f);
	...
	public CharacterFollowCam(CameraManager camMngr)
	{
		mngr = camMngr;
	...
	public void Enter(int from)
	{
		if (prevTarget != mngr.Target)
		{
			...
			centerOffset = regularOffset;
			UnitController component = mngr.Target.gameObject.GetComponent<UnitController>();
			if (component != null)
			{
				if (component.unit.IsImpressive && component.unit.Data.UnitSizeId == UnitSizeId.LARGE)
				{
					centerOffset = impressiveOffeset;
				}
				else if (component.unit.Data.WarbandId == WarbandId.SKAVENS)
				{
					centerOffset = skavenOffset;
				}
			}
		...
		snappingTime = 0f;
		if (PandoraSingleton<PandoraInput>.Instance.GetAxis("v") == 0f && PandoraSingleton<PandoraInput>.Instance.GetAxis("h") == 0f)
		{
			isSnapping = true;
		}
	...
	public void Update()
	{
		...
		if (!isSnapping)
		{
			dummyCam.position = vector + vector2 * mngr.Zoom;
		}
		float num4 = 0.2f;
		if (Physics.SphereCast(vector, num4, vector2, out hit, mngr.Zoom, LayerMaskManager.groundMask) && hit.transform != mngr.Target)
		{
