...
public class test_camera : MonoBehaviour
{
	...
	{
		float num = 0.01f;
		if (PandoraSingleton<PandoraInput>.Instance.GetAxis("Button1") > 0f)
		{
		...
		if (PandoraSingleton<PandoraInput>.Instance.GetAxis("MouseWheel") < 0f - num || PandoraSingleton<PandoraInput>.Instance.GetAxis("MouseWheel") > num)
		{
			desiredDistance = Mathf.Clamp(Distance - PandoraSingleton<PandoraInput>.Instance.GetAxis("MouseWheel") * MouseWheelSensitivity, DistanceMin, DistanceMax);
		}
	}
