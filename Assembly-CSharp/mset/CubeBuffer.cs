	...
	public class CubeBuffer
	{
		...
		public void sampleBicubic(ref Color dst, float u, float v, int face)
		{
			...
			for (int k = 0; k < 4; k++)
			{
				b2 = cubicKernel[0, k];
				a = cubicKernel[1, k];
				color = cubicKernel[2, k];
				b3 = cubicKernel[3, k];
				b2 = Color.Lerp(a, b2, t);
				b3 = Color.Lerp(color, b3, t);
				b2 = a + b * (a - b2);
				b3 = color + b * (color - b3);
				a2 = Color.Lerp(a, b2, u);
				color2 = Color.Lerp(b2, b3, u);
				b4 = Color.Lerp(b3, color, u);
				a2 = Color.Lerp(a2, color2, u);
				color2 = Color.Lerp(color2, b4, u);
				cubicKernel[0, k] = Color.Lerp(a2, color2, u);
			}
			b2 = cubicKernel[0, 0];
			a = cubicKernel[0, 1];
			color = cubicKernel[0, 2];
			b3 = cubicKernel[0, 3];
			b2 = Color.Lerp(a, b2, t);
			b3 = Color.Lerp(color, b3, t);
			b2 = a + b * (a - b2);
			b3 = color + b * (color - b3);
			a2 = Color.Lerp(a, b2, v);
			color2 = Color.Lerp(b2, b3, v);
			b4 = Color.Lerp(b3, color, v);
			a2 = Color.Lerp(a2, color2, v);
			color2 = Color.Lerp(color2, b4, v);
			dst = Color.Lerp(a2, color2, v);
		}
			...
			for (ulong num2 = 0uL; num2 < 6; num2++)
			{
				for (ulong num3 = 0uL; num3 < num; num3++)
				{
					for (ulong num4 = 0uL; num4 < num; num4++)
					{
						...
						pixels[num2 * num * num + num3 * num + num4] = tex.GetPixelBilinear(pano_u, pano_v);
					}
				}
			}
