...
namespace mset
{
	public class Util
	{
		...
		{
			float num = 2f / (float)(double)faceSize;
			float num2 = ((float)(double)col + 0.5f) * num - 1f;
			float num3 = ((float)(double)row + 0.5f) * num - 1f;
		...
		{
			float num = 0.5f;
			float num2 = ((float)(double)col + num) / (float)(double)width;
			float num3 = ((float)(double)row + num) / (float)(double)height;
			float f = MathF.PI * -2f * num2 - MathF.PI / 2f;
			float f2 = MathF.PI / 2f * (2f * num3 - 1f);
			cosPhi = Mathf.Cos(f2);
		...
		public static void cubeToLatLongLookup(ref float pano_u, ref float pano_v, ulong face, ulong col, ulong row, ulong faceSize)
		{
			...
			pano_v = Mathf.Asin(dst.y) / MathF.PI + 0.5f;
			pano_u = 0.5f * Mathf.Atan2(0f - dst.x, 0f - dst.z) / MathF.PI;
		...
		{
			dir = dir.normalized;
			u = 0.5f * Mathf.Atan2(0f - dir.x, 0f - dir.z) / MathF.PI;
			u = Mathf.Repeat(u, 1f);
