...
namespace Pathfinding
{
	...
	public class SimpleSmoothModifier : MonoModifier
	{
			...
			if (strength > 0f)
			{
				for (int m = 0; m < iterations; m++)
				{
					...
					for (int n = 1; n < list.Count - 1; n++)
					{
						...
						list[n] = Vector3.Lerp(vector, (a2 + list[n + 1]) / 2f, strength);
						a2 = vector;
					}
				}
			}
		...
		public List<Vector3> SmoothBezier(List<Vector3> path)
		{
			...
			for (int i = 0; i < path.Count - 1; i++)
			{
				Vector3 b = (i != 0) ? (path[i + 1] - path[i - 1]) : (path[i + 1] - path[i]);
				Vector3 b2 = (i != path.Count - 2) ? (path[i] - path[i + 2]) : (path[i] - path[i + 1]);
				b *= bezierTangentLength;
				b2 *= bezierTangentLength;
				Vector3 vector = path[i];
				Vector3 p = vector + b;
				Vector3 vector2 = path[i + 1];
				Vector3 p2 = vector2 + b2;
				for (int j = 0; j < num; j++)
				{
					list.Add(AstarSplines.CubicBezier(vector, p, p2, vector2, (float)j / (float)num));
				}
			}
			list.Add(path[path.Count - 1]);
			return list;
