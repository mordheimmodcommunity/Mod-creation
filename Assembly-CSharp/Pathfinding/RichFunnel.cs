...
namespace Pathfinding
{
    public class RichFunnel : RichPathPart
    {
        ...
        public Vector3 Update(Vector3 position, List<Vector3> buffer, int numCorners, out bool lastCorner, out bool requiresRepath)
        {
            ...
            if (ClampToNavmeshInternal(ref position))
            {
                requiresRepath = true;
                lastCorner = false;
                buffer.Add(position);
        ...
        private bool ClampToNavmeshInternal(ref Vector3 position)
        {
            ...
            int i = currentNode + 1;
            for (int num = Math.Min(currentNode + 3, nodes.Count); i < num; i++)
            {
                if (nodes[i].Destroyed)
                {
                    return true;
                }
                if (nodes[i].ContainsPoint(p))
                {
                    currentNode = i;
                    return false;
                }
            ...
            for (int count = nodes.Count; j < count; j++)
            {
                ...
                Vector3 a = nodes[j].ClosestPointOnNode(position);
