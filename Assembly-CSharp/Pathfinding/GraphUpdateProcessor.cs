    ...
    internal class GraphUpdateProcessor
    {
        ...
        private void QueueGraphUpdatesInternal()
        {
            ...
            while (graphUpdateQueue.Count > 0)
            {
                ...
                foreach (IUpdatableGraph updateableGraph in astar.astarData.GetUpdateableGraphs())
                {
                    NavGraph graph = updateableGraph as NavGraph;
                    if (graphUpdateObject.nnConstraint == null || graphUpdateObject.nnConstraint.SuitableGraph(astar.astarData.GetGraphIndex(graph), graph))
                    {
