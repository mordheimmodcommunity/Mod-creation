    ...
    public class RaycastModifier : MonoModifier
    {
        ...
        public override void Apply(Path p)
        {
            ...
            for (int i = 0; i < iterations; i++)
            {
                ...
                int num = 0;
                while (num < vectorPath.Count - 2)
                {
                    Vector3 v = vectorPath[num];
                    Vector3 v2 = vectorPath[num + 2];
                    if (ValidateLine(null, null, v, v2))
                    {
                        vectorPath.RemoveAt(num + 1);
                    }
                    else
                    {
                        num++;
        ...
        private static void Subdivide(List<Vector3> points)
        {
            ...
            for (int num = count - 1; num > 0; num--)
            {
                ...
                points[num * 3] = points[num];
                if (num != count - 1)
                {
                    points[num * 3 + 1] = Vector3.Lerp(a, b, 0.33f);
                    points[num * 3 + 2] = Vector3.Lerp(a, b, 0.66f);
                }
            }
        }
        ...
        public bool ValidateLine(GraphNode n1, GraphNode n2, Vector3 v1, Vector3 v2)
        {
            if (useRaycasting)
            {
                ...
                else if (Physics.Linecast(v1 + raycastOffset, v2 + raycastOffset, mask))
                {
                    return false;
                }
            }
