...
namespace Pathfinding.Voxels
{
	public class Voxelize
	{
		...
		public void SimplifyContour(List<int> verts, List<int> simplified, float maxError, int maxEdgeLenght, int buildFlags)
		{
			...
			if (num23 > 0f && (buildFlags & 3) != 0)
			{
				...
				while (num24 < simplified.Count / 4 && simplified.Count / 4 <= 200)
				{
					...
					bool flag4 = false;
					if ((buildFlags & 1) == 1 && (verts[num33 * 4 + 3] & 0xFFFF) == 0)
					{
						flag4 = true;
					}
					if ((buildFlags & 2) == 1 && (verts[num33 * 4 + 3] & 0x20000) == 1)
					{
						flag4 = true;
					}
					if (flag4)
		...
		public void BuildPolyMesh(VoxelContourSet cset, int nvp, out VoxelMesh mesh)
		{
			...
			for (int i = 0; i < cset.conts.Count; i++)
			{
				...
				if (voxelContour.nverts >= 3)
				{
					...
					num3 = Math.Max(val, voxelContour4.nverts);
				}
			}
			if (num >= 65534)
			{
			...
			for (int j = 0; j < cset.conts.Count; j++)
			{
				...
				if (voxelContour5.nverts >= 3)
				{
					...
					int num9 = Triangulate(voxelContour5.nverts, voxelContour5.verts, ref indices, ref tris);
					int num10 = num6;
					for (int l = 0; l < num9 * 3; l++)
					{
						array2[num7] = tris[l] + num10;
						num7++;
					}
					for (int m = 0; m < num9; m++)
					{
						array3[num8] = voxelContour5.area;
						num8++;
					}
					for (int n = 0; n < voxelContour5.nverts; n++)
					{
						array[num6] = new Int3(voxelContour5.verts[n * 4], voxelContour5.verts[n * 4 + 1], voxelContour5.verts[n * 4 + 2]);
						num6++;
					}
				}
			}
			mesh = default(VoxelMesh);
			Int3[] array4 = new Int3[num6];
			for (int num11 = 0; num11 < num6; num11++)
			{
				array4[num11] = array[num11];
			}
			int[] array5 = new int[num7];
			int[] array6 = new int[num8];
			Buffer.BlockCopy(array2, 0, array5, 0, num7 * 4);
			Buffer.BlockCopy(array3, 0, array6, 0, num8 * 4);
			mesh.verts = array4;
			mesh.tris = array5;
			mesh.areas = array6;
		}
		...
		public void VoxelizeInput()
		{
			...
			int num = (int)(size.y / cellHeight);
			float num2 = 1f / cellSize;
			float num3 = 1f / cellHeight;
			float num4 = Mathf.Cos(Mathf.Atan(Mathf.Tan(maxSlope * (MathF.PI / 180f)) * (num3 * cellSize)));
			...
			{
				num5 = Math.Max(inputMeshes[i].vertices.Length, num5);
			}
			Vector3[] array6 = new Vector3[num5];
			Matrix4x4 lhs = Matrix4x4.TRS(-new Vector3(0.5f, 0f, 0.5f), Quaternion.identity, Vector3.one) * Matrix4x4.Scale(new Vector3(num2, num3, num2)) * Matrix4x4.TRS(-a, Quaternion.identity, Vector3.one);
			for (int j = 0; j < inputMeshes.Count; j++)
			{
				...
				for (int l = 0; l < num6; l += 3)
				{
					...
					float num7 = Vector3.Dot(Vector3.Cross(vector2 - vector, vector3 - vector).normalized, Vector3.up);
					int area2 = (!(num7 < num4)) ? (1 + area) : 0;
					...
					{
						int num8 = clipper.ClipPolygon(array, 3, array2, 1f, (float)(-m) + 0.5f, 0);
						if (num8 < 3)
						{
							continue;
						}
						num8 = clipper.ClipPolygon(array2, num8, array3, -1f, (float)m + 0.5f, 0);
						if (num8 < 3)
						{
							continue;
						}
						...
						int num11 = Mathf.Clamp((int)Math.Round(num9), 0, voxelArea.depth - 1);
						int num12 = Mathf.Clamp((int)Math.Round(num10), 0, voxelArea.depth - 1);
						for (int num13 = num11; num13 <= num12; num13++)
						{
							int num14 = clipper.ClipPolygon(array3, num8, array4, 1f, (float)(-num13) + 0.5f, 2);
							if (num14 < 3)
							{
								continue;
							}
							num14 = clipper.ClipPolygonY(array4, num14, array5, -1f, (float)num13 + 0.5f, 2);
							if (num14 >= 3)
							{
								...
								int num18 = (int)Math.Ceiling(num16);
								if (num18 >= 0 && num15 <= (float)num)
								{
									int num19 = Math.Max(0, (int)num15);
									num18 = Math.Max(num19 + 1, num18);
									voxelArea.AddLinkedSpan(num13 * voxelArea.width + m, (uint)num19, (uint)num18, area2, voxelWalkableClimb);
								}
							}
						}
					}
			...
			while (num2 < num)
			{
				for (int i = 0; i < voxelArea.width; i++)
				{
					...
					while (num4 != -1 && linkedSpans[num4].bottom != uint.MaxValue)
					{
						...
						{
							Debug.Log(top + " " + num5);
							Debug.DrawLine(new Vector3((float)i * cellSize, (float)(double)top * cellHeight, (float)num3 * cellSize) + min, new Vector3((float)i * cellSize, (float)(double)num5 * cellHeight, (float)num3 * cellSize) + min, Color.yellow, 1f);
						}
		...
		private void DrawLine(int a, int b, int[] indices, int[] verts, Color col)
		{
			...
			Debug.DrawLine(VoxelToWorld(verts[num], verts[num + 1], verts[num + 2]), VoxelToWorld(verts[num2], verts[num2 + 1], verts[num2 + 2]), col);
		}
		...
		public Int3 VoxelToWorldInt3(Int3 voxelPosition)
		{
			...
			lhs = new Int3(x, y, Mathf.RoundToInt(num3 * vector3.z));
			return lhs + (Int3)voxelOffset;
		}
