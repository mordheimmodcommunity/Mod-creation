	...
	internal class RecastMeshGatherer
	{
		...
		public void CollectTerrainMeshes(bool rasterizeTrees, float desiredChunkSize, List<RasterizationMesh> result)
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				{
					GenerateTerrainChunks(array[i], bounds, desiredChunkSize, result);
					if (rasterizeTrees)
					{
						CollectTreeMeshes(array[i], result);
					}
				}
			}
		}
		...
		private void GenerateTerrainChunks(Terrain terrain, Bounds bounds, float desiredChunkSize, List<RasterizationMesh> result)
		{
			...
			for (int i = 0; i < heightmapHeight; i += num2)
			{
				for (int j = 0; j < heightmapWidth; j += num)
				{
					...
					RasterizationMesh item = GenerateHeightmapChunk(heights, heightmapScale, position, j, i, width, depth, terrainSampleSize);
					result.Add(item);
				}
			}
		}
		...
		public void CollectColliderMeshes(List<RasterizationMesh> result)
		{
			...
			if (tagMask.Count > 0 || (int)mask != 0)
			{
				...
				{
					if (((((int)mask >> collider.gameObject.layer) & 1) != 0 || tagMask.Contains(collider.tag)) && collider.enabled && !collider.isTrigger && collider.bounds.Intersects(bounds) && collider.GetComponent<RecastMeshObj>() == null)
					{
		...
		private RasterizationMesh RasterizeCollider(Collider col, Matrix4x4 localToWorldMatrix)
		{
			...
			else if (col is SphereCollider || col is CapsuleCollider)
			{
				...
				Matrix4x4 rhs = Matrix4x4.TRS((!(sphereCollider != null)) ? capsuleCollider.center : sphereCollider.center, Quaternion.identity, Vector3.one * num);
				rhs = localToWorldMatrix * rhs;
				result = RasterizeCapsuleCollider(num, height, col.bounds, rhs);
			}
		...
		{
			Matrix4x4 rhs = Matrix4x4.TRS(collider.center, Quaternion.identity, collider.size * 0.5f);
			rhs = localToWorldMatrix * rhs;
			return new RasterizationMesh(BoxColliderVerts, BoxColliderTris, collider.bounds, rhs);
		}
		...
		private RasterizationMesh RasterizeCapsuleCollider(float radius, float height, Bounds bounds, Matrix4x4 localToWorldMatrix)
		{
			...
			if (capsuleCache == null)
			{
				...
				for (int j = 0; j < num; j++)
				{
					...
					{
						array[k + j * num2] = new Vector3(Mathf.Cos((float)k * MathF.PI * 2f / (float)num2) * Mathf.Sin((float)j * MathF.PI / (float)(num - 1)), Mathf.Cos((float)j * MathF.PI / (float)(num - 1)) + ((j >= num / 2) ? (0f - height) : height), Mathf.Sin((float)k * MathF.PI * 2f / (float)num2) * Mathf.Sin((float)j * MathF.PI / (float)(num - 1)));
					}
				}
