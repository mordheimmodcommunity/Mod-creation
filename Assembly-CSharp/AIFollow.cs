...
public class AIFollow : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        Vector3 a = path[pathIndex];
        Vector3 position = tr.position;
        a.y = position.y;
        while ((a - tr.position).sqrMagnitude < pickNextWaypointDistance * pickNextWaypointDistance)
        {
            pathIndex++;
            if (pathIndex >= path.Length)
            {
                if ((a - tr.position).sqrMagnitude < pickNextWaypointDistance * targetReached * (pickNextWaypointDistance * targetReached))
                {
                    ReachedEndOfPath();
                    return;
                }
