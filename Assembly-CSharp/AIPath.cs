...
public class AIPath : MonoBehaviour
{
	...
	protected Vector3 CalculateVelocity(Vector3 currentPosition)
	{
		...
		{
			float num = VectorMath.SqrDistanceXZ(vectorPath[currentWaypointIndex], currentPosition);
		...
		Vector3 vector = vectorPath[currentWaypointIndex - 1];
		Vector3 vector2 = vectorPath[currentWaypointIndex];
		float value = VectorMath.LineCircleIntersectionFactor(To2D(currentPosition), To2D(vector), To2D(vector2), pickNextWaypointDist);
		value = Mathf.Clamp01(value);
		Vector3 a = Vector3.Lerp(vector, vector2, value);
		Vector3 vector3 = a - currentPosition;
		vector3.y = 0f;
		float magnitude = vector3.magnitude;
		float num2 = (!(slowdownDistance > 0f)) ? 1f : Mathf.Clamp01(magnitude / slowdownDistance);
