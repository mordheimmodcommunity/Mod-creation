...
namespace Pathfinding.Examples
{
	...
	public class ProceduralWorld : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			int x = Mathf.RoundToInt((position.x - tileSize * 0.5f) / tileSize);
			Vector3 position2 = target.position;
			Int2 @int = new Int2(x, Mathf.RoundToInt((position2.z - tileSize * 0.5f) / tileSize));
			...
			while (flag)
			{
				...
				{
					Int2 key = tile.Key;
					if (Mathf.Abs(key.x - @int.x) <= range)
					{
						Int2 key2 = tile.Key;
						if (Mathf.Abs(key2.y - @int.y) <= range)
						{
							continue;
						}
					}
