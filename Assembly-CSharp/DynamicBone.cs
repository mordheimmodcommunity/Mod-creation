...
public class DynamicBone : MonoBehaviour
{
    ...
    private void InitTransforms()
    {
        for (int i = 0; i < m_Particles.Count; i++)
        {
            if (m_Particles[i].m_Transform != null)
            {
                m_Particles[i].m_Transform.localPosition = m_Particles[i].m_InitLocalPosition;
                m_Particles[i].m_Transform.localRotation = m_Particles[i].m_InitLocalRotation;
            }
        }
    }
    ...
    private void ResetParticlesPosition()
    {
        for (int i = 0; i < m_Particles.Count; i++)
        {
            if (m_Particles[i].m_Transform != null)
            {
                m_Particles[i].m_Position = (m_Particles[i].m_PrevPosition = m_Particles[i].m_Transform.position);
                continue;
            }
            Transform transform = m_Particles[m_Particles[i].m_ParentIndex].m_Transform;
            m_Particles[i].m_Position = (m_Particles[i].m_PrevPosition = transform.TransformPoint(m_Particles[i].m_EndOffset));
        }
    }
    ...
    private void UpdateParticles1()
    {
        ...
        for (int i = 0; i < m_Particles.Count; i++)
        {
            ...
            if (particle.m_ParentIndex >= 0)
            {
                ...
                Vector3 b = m_ObjectMove * particle.m_Inert;
                particle.m_PrevPosition = particle.m_Position + b;
                particle.m_Position += a * (1f - particle.m_Damping) + gravity + b;
            }
            else
            {
    ...
    private void UpdateParticles2()
    {
        for (int i = 1; i < m_Particles.Count; i++)
        {
            ...
            if (num2 > 0f || particle.m_Elasticity > 0f)
            {
                ...
                Vector3 a = (!(particle.m_Transform != null)) ? localToWorldMatrix.MultiplyPoint3x4(particle.m_EndOffset) : localToWorldMatrix.MultiplyPoint3x4(particle.m_Transform.localPosition);
    ...
    private void SkipUpdateParticles()
    {
        for (int i = 0; i < m_Particles.Count; i++)
        {
            ...
            if (particle.m_ParentIndex >= 0)
            {
                Vector3 vector = m_ObjectMove * particle.m_Inert;
